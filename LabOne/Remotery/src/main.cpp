...
int main(int argc, char* argv[])
{
  ...
  {                    // Loop over image rows
    rmt_BeginCPUSample(Rows, 0);
    rmt_LogText("Loop Rows Start");
    ...
    {// Loop cols
      rmt_BeginCPUSample(COLS, 0);
      rmt_LogText("Loop Cols Start");
      ...
      for (size_t sy = 0, i = (height - y - 1) * width + x; sy < 2; sy++) // 2x2 subpixel rows
      {
        ...
        { // 2x2 subpixel cols
          ...
          {
            rmt_BeginCPUSample(SUBPIXEL, 0);
            rmt_LogText("Doing Samples");
